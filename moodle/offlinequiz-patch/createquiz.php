...
if ($downloadall && $offlinequiz->docscreated) {
    ...
    $combinedpath = clean_filename(get_string('combinedforms', 'offlinequiz'));
    ...
    foreach ($groups as $group) {
        ...
        if ($combinedfile = offlinequiz_get_pdf_combined($group, $context)) {
            $filelist[$combinedpath . '/' . $combinedfile->get_filename()] = $combinedfile;
        }
...
if ($mode == 'preview') {
    ...
    if (!$forcepdfnew) {
        ...
        // O============================================================.
        // O Show/create the combined question + answer forms (PDF only).
        // O============================================================.
        if ($offlinequiz->fileformat != OFFLINEQUIZ_DOCX_FORMAT && $offlinequiz->fileformat != OFFLINEQUIZ_LATEX_FORMAT) {
            echo $OUTPUT->box_start('generalbox linkbox docsbox');
            ...
            foreach ($groups as $group) {
                ...
                if (!$combinedfile) {
                    $combinedfile = offlinequiz_create_pdf_combined($offlinequiz, $group, $context);
                }

                if ($combinedfile) {
                    $url = "$CFG->wwwroot/pluginfile.php/" . $combinedfile->get_contextid() . '/' .
                            $combinedfile->get_component() . '/' . $combinedfile->get_filearea() . '/' .
                            $combinedfile->get_itemid() . '/' . $combinedfile->get_filename() . '?forcedownload=1';
                    echo $OUTPUT->action_link($url, get_string('combinedformforgroup', 'offlinequiz', $groupletter));
                    echo '<br />&nbsp;<br />';
                    @flush();
                    @ob_flush();
                } else {
