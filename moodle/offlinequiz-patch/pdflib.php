...
/**
 * Returns the file name prefix of the combined (question form + answer form) PDF of a group.
 *
 * @param object $group the offline group object
 * @return string
 */
function offlinequiz_get_combined_pdf_prefix($group) {
    $letterstr = 'ABCDEFGHIJKL';
    return get_string('fileprefixcombined', 'offlinequiz') . '_' . $letterstr[$group->groupnumber - 1] . '_';
}

/**
 * Finds the already generated combined PDF (question form followed by answer form) of a group.
 *
 * @param object $group the offline group object
 * @param object $context the context of the offline quiz.
 * @return stored_file|bool the file or false if it does not exist.
 */
function offlinequiz_get_pdf_combined($group, $context) {
    $fs = get_file_storage();
    $prefix = offlinequiz_get_combined_pdf_prefix($group);
 ...
 */
function offlinequiz_create_pdf_combined($offlinequiz, $group, $context) {
    global $CFG;
    require_once($CFG->dirroot . '/mod/assign/feedback/editpdf/classes/pdf.php');

    $fs = get_file_storage();
    $questionfile = $fs->get_file($context->id, 'mod_offlinequiz', 'pdfs', 0, '/', $group->questionfilename);
    $answerfile = $fs->get_file($context->id, 'mod_offlinequiz', 'pdfs', 0, '/', $group->answerfilename);
    if (!$questionfile || !$answerfile) {
        return false;
    }

    // Remove combined files left over from a previous run.
    while ($oldfile = offlinequiz_get_pdf_combined($group, $context)) {
        $oldfile->delete();
    }
